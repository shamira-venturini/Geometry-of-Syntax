...
def build_assignment(
    ...
    for target_index, target_sig in enumerate(target_signatures):
        target_verb = str(target_sig["verb"])
    ...
    if int(np.sum(matched_columns != -1)) != n_rows:
        raise RuntimeError("Failed to find a full lexical-overlap-free prime assignment.")
...
def main() -> None:
    ...
    target_signatures, prime_signatures = build_signatures(source_frame)
    ...
    summary = {
        ...
        "notes": [
            ...
            "The original constrained corpus is retained separately as the lexical-overlap comparison condition.",
        ],
    }
